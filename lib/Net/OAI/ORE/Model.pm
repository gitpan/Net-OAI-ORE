...
Net::OAI::ORE::Model->mk_accessors(qw(die_level));

=head1 METHODS

=head2 CREATION AND MODIFICATION

=head3 Net::OAI::ORE::Model->new(%args) or Net::OAI::ORE::Model->new($rdf_model)
...
If $rem is supplied then this is expected to be a Net::OAI::ORE::ReM object
...
the methods provided via L<Net::OAI::ORE::ReM>::serialize will be used.
...
See L<Net::OAI::ORE::N3> for "pretty printing" methods.
...
sub as_n3 {
  ...
  while ($statement) {
    ...
    if ($obj->isa('RDF::Core::Resource')) {
      ...
      $object='"'.Net::OAI::ORE::N3::_n3_escape($object).'"';
    }
